...
public class ContentTransferCheckDialog extends Dialog {

	final Logger logger = LoggerFactory.getLogger(getClass());

	@Inject
	public ContentTransferCheckDialog(@Optional @Named(IServiceConstants.ACTIVE_SHELL) Shell parentShell) {
		super(parentShell);
	}
	...
	private Text remoteDirText;
	private Text userPassText;
	private Text fileNmText;
	...
	public void create() {
		...
		if(ftpIpText != null && !ftpIpText.isDisposed()) {
			...
			target = WidgetProperties.text(SWT.Modify).observe(fileNmText);
			model = BeanProperties.value("orgFileNm").observe(transfer);
			ctx.bindValue(target, model);
			
			target = WidgetProperties.text(SWT.Modify).observe(remoteDirText);
	...
	protected Control createDialogArea(Composite parent) {
		...
		Label remoteDir = new Label(container, SWT.NONE);
		remoteDir.setText("리모트 디렉토리"); // 아이디
		
		remoteDirText = new Text(container, SWT.BORDER);
		GridData remoteDirGd = new GridData(GridData.FILL_HORIZONTAL);
		remoteDirGd.horizontalSpan = 3;
		remoteDirText.setLayoutData(remoteDirGd);
		...
		fileNmText = new Text(container, SWT.BORDER);
		GridData gd_fileNmText = new GridData(GridData.FILL_HORIZONTAL);
		gd_fileNmText.horizontalSpan = 3;
		fileNmText.setLayoutData(gd_fileNmText);

		fileBtn = new Button(container, SWT.PUSH);
		fileBtn.setText("Browse");
		fileBtn.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				FileDialog fg = new FileDialog(container.getShell(), SWT.OPEN);

				// 입력된 경로가 있다면 해당 경로를 표시한다.
				if(!StringUtils.isBlank(transfer.getFlPath())) {
					fg.setFilterPath(transfer.getFlPath());
				}

				// 팝업창 타이틀명
				fg.setText("\uC791\uC5C5\uD3F4\uB354 \uC120\uD0DD"); // 작업폴더 선택
				String fileName = fg.open();
				if (fileName != null) {
					// Set the text box to the new selection
					fileNmText.setText(fileName.replaceAll("\\\\", "/"));
				}
			}
		});
		...
		gd_progressBar.horizontalSpan = 4;
	...
	protected Point getInitialSize() {
		return new Point(450, 310);
	}

	@Inject @Optional
	public void  getEvent(@UIEventTopic(TransferEventConstants.TOPIC_SAMPLE) final Transfer transfer) {
		if(logger.isDebugEnabled()) {
			logger.debug("sample: "+transfer.getProgress());
		}

		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				if(transfer.getProgress() == 100 || Integer.valueOf(transfer.getWorkStatcd()) > 3) {
	...
	protected void okPressed() {
		if(transferService != null) {
			transfer.setTransferType(TMode.SAMPLE);
			Job job = new Job("Transfer Sample Job") {
				@Override
				protected IStatus run(IProgressMonitor monitor) {
					if(transferService.uploadCheck(transfer)) {
						Display.getDefault().asyncExec(new Runnable() {
							public void run() {
						...
						try {
							...
							prefs.put("remoteDir", transfer.getRemoteDir());
							prefs.flush();
							//prefs.sync();
